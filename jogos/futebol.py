...
def loopinicial():
    while True:
        system("clear")
        resposta = input("""    
            Se deseja buscar os jogos de hoje digite B 
            
            Se já tiver feito uma busca hoje digite L para listar
...
def listadados():
    ...
    data = tablib.Dataset(*jogos, headers=cabecalhos)
    return data


def exportadados():
    data = listadados()
    with open('jogos.xls', 'wb') as f:
